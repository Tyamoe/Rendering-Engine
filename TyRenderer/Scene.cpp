...
Scene::Scene()
{
	...
	*/
	...
	//
	Model* m = new Model("./resources/models/cube.obj", PixelColorF(0.64f, 0.0f, 0.64f), 0.0f, 0.0f);
	geometry.push_back(m); 
	BVH* bvh = BVH::GenerateBVH(m);
	
	m->bvh = bvh;

	//Model* m1 = new Model("./resources/models/fuk.obj", PixelColorF(0.0f, 0.74f, 0.0f), 0.0f, 0.0f);
	//geometry.push_back(m1);
	//BVH* bvh1 = BVH::GenerateBVH(m1);
	//
	//m1->bvh = bvh1;
